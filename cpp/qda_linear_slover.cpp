...
qdals_res qda_linear_solver(MatrixXcd A, VectorXcd b, double T=500, int M=200) {
    ...
    b = b / b.norm();
    int aqc_n =  n+1;
    cout << "The number of qubits is: " << aqc_n << endl;
    ...
    VectorXcd current_state = VectorXcd::Zero( b.size()*2);
    ...
    for (double m = 0; m < M; m++) {
        ...
        for (int i = 0; i < DM; i++) {
            ...
            auto circuit1 = block_encoding_circuit(A1);
            // cout << "The matrix A1 is: \n" << A1 << endl;
            // cout << "The matrix U1 is: \n" << U1.block(0, 0, 4, 4) << endl;
            circuit << circuit1; // 有可能是加反了
            ...
            auto circuit0 = block_encoding_circuit(A0);
            secendcircuit <<circuit0;
...
int main() {
    MatrixXcd A(3, 3);
    A << Complex(2, 0), Complex(1, 0), Complex(-1, 0),
          Complex(1, 0),  Complex(0.5, -1), Complex(1, 0),
          Complex(-1, 0), Complex(1, 0), Complex(1, 0)
          ;
    VectorXcd b(3);
    b << Complex(3, 0), Complex(1, 0), Complex(1, 0);
